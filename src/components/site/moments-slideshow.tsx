                ...
                {[...fullerItems, ...fullerItems].map((moment, itemIndex) => {
                  ...
                  return (
                    <Card
                      ...
                      className="photo-card film-card group/photo w-[72vw] shrink-0 border border-zinc-700 bg-[#121212] p-3 text-zinc-100 shadow-[0_14px_30px_rgba(0,0,0,0.34)] sm:w-[24rem]"
                    >
                      <div className="space-y-3">
                        ...
                        <div className="px-1 pb-1 text-left">
                          <p className="text-[0.62rem] font-semibold tracking-[0.24em] text-zinc-400 uppercase">{moment.label ?? `${locationLabel} Highlight`}</p>
                          <CardTitle className="mt-1 text-lg text-zinc-100">{moment.title ?? deriveTitle(moment.image, sourceIndex)}</CardTitle>
                          {moment.description ? (
                            <CardDescription className="mt-1 text-sm leading-relaxed text-zinc-400">{moment.description}</CardDescription>
                          ) : null}
